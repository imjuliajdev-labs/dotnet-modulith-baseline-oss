...
public sealed class IntegrationEventLifecycleGuardrailTests
{
    [Fact]
    public void EveryPublishedIntegrationEventDeclaresLifecycleMetadata()
    {
        var missing = DiscoverPublishedIntegrationEvents()
        ...
        Assert.True(
            missing.Length == 0,
            $"Integration events missing [ContractLifecycle]: {string.Join(", ", missing)}");
    }

    [Fact]
    public void LifecycleDatesAreIso8601LocalDates()
    {
    ...
    [Fact]
    public void SupersededEventsDeclareReplacementAndRetireDate()
    {
    ...
    [Fact]
    public void ActiveEventsDoNotDeclareSupersedeMetadata()
    {
    ...
    [Fact]
    public void SupersedeChainTargetsAreIntegrationEventsInPublicContracts()
    {
        var registry = DiscoverPublishedIntegrationEvents().ToHashSet();
        ...
        foreach (var (type, lifecycle) in EnumerateLifecycles().Where(static pair => pair.Lifecycle.SupersededBy is not null))
        {
            ...
            Assert.True(
                typeof(IIntegrationEvent).IsAssignableFrom(successor),
                $"{type.FullName}: SupersededBy '{successor.FullName}' does not implement IIntegrationEvent.");
            Assert.True(
                registry.Contains(successor),
                $"{type.FullName}: SupersededBy '{successor.FullName}' is not a registered public integration event under *.PublicContracts.Events.");
            Assert.NotEqual(type, successor);
        }
    }

    [Fact]
    public void SupersededEventsRetireWithinCompatibilityWindowOfSuccessor()
    {
        foreach (var (type, lifecycle) in EnumerateLifecycles().Where(static pair => pair.Lifecycle.Status == ContractLifecycleStatus.Superseded))
        {
            var successor = lifecycle.SupersededBy!;
            ...
            Assert.True(ContractLifecyclePolicy.TryParseIsoDate(lifecycle.IntroducedOn, out var introducedOn), $"{type.FullName}: IntroducedOn unparseable.");
            Assert.True(ContractLifecyclePolicy.TryParseIsoDate(lifecycle.RetireOn!, out var retireOn), $"{type.FullName}: RetireOn unparseable.");
            Assert.True(ContractLifecyclePolicy.TryParseIsoDate(successorLifecycle!.IntroducedOn, out var successorIntroducedOn), $"{successor.FullName}: IntroducedOn unparseable.");
    ...
    [Fact]
    public void NoSupersededEventHasExceededItsRetirementDate()
    {
        ...
        Assert.True(
            expired.Length == 0,
            $"Integration events past their retirement deadline (today={today:O}): {string.Join(", ", expired)}. Remove the type and its consumers, then update successors before re-running this gate.");
    }
    ...
    [Fact]
    public void AtMostOneActiveVersionExistsPerEventBaseName()
    {
    ...
    {
        foreach (var type in DiscoverPublishedIntegrationEvents())
        {
    ...
    private static IReadOnlyList<Type> DiscoverPublishedIntegrationEvents()
    {
        ...
        foreach (var assembly in RepositoryFiles.ReadModulePublicContractsAssemblies())
        {
            foreach (var type in assembly.GetTypes())
            {
                if (type is { IsClass: true, IsAbstract: false }
                    ...
                    && ns.EndsWith(".PublicContracts.Events", StringComparison.Ordinal))
                {
                    results.Add(type);
                }
            }
