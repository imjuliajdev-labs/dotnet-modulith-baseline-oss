...
namespace Integration.Tests.ModuleCoverage.Blog;

public sealed class BlogBootstrapManifestTests
{
    [Xunit.Fact]
    public async Task PlatformBootstrapManifestIncludesTheScaffoldedModule()
    {
        ...
        Xunit.Assert.Contains(modules, element =>
            string.Equals(element.GetProperty("key").GetString(), "blog", StringComparison.Ordinal)
            && string.Equals(element.GetProperty("displayName").GetString(), "Blog", StringComparison.Ordinal));
    }
}
