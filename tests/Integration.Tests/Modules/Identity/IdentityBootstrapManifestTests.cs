...
namespace Integration.Tests.ModuleCoverage.Identity;

public sealed class IdentityBootstrapManifestTests
{
    [Xunit.Fact]
    public async Task PlatformBootstrapManifestIncludesTheScaffoldedModule()
    {
        ...
        Xunit.Assert.Contains(modules, element =>
            string.Equals(element.GetProperty("key").GetString(), "identity", StringComparison.Ordinal)
            && string.Equals(element.GetProperty("displayName").GetString(), "Identity", StringComparison.Ordinal));
    }
}
