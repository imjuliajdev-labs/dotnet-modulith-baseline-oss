...
public sealed class KnowledgeBaseBootstrapManifestTests
{
    [Xunit.Fact]
    public async Task PlatformBootstrapManifestIncludesTheScaffoldedModule()
    {
        ...
        Xunit.Assert.Contains(modules, element =>
            string.Equals(element.GetProperty("key").GetString(), "knowledge-base", StringComparison.Ordinal)
            && string.Equals(element.GetProperty("displayName").GetString(), "Knowledge Base", StringComparison.Ordinal));
    }
}
